...
const BodySchema = z.object({ intent: z.string().min(8).max(2_000) });
...
export async function POST(req: NextRequest) {
  ...
  const [existingAgents, existingSkills, config] = await Promise.all([
    loadAgents(),
    listSkills(),
    loadConfig(),
  ]);
  const engine = getEngine(config.engineMode);
  const system = buildAgentGenerateSystem({
    memory,
    existingAgents,
    existingSkills,
  });
  const user = buildAgentGenerateUser({ intent: parsed.data.intent });
